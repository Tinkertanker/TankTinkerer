...
public class SerialController : MonoBehaviour
{
	...
	[Tooltip("Tells the serial controller which team it is controlling")]
	public string team;
	...
	//Initialize variables that can be obtained by other gameobjects
	[HideInInspector]
	public float[] m_TurnValues = new float[] { 0, 0 };
	[HideInInspector]
	public float[] m_MoveValues = new float[] { 0, 0 };
	[HideInInspector]
	public float[] m_AimValues = new float[] { 0, 0 };
	[HideInInspector]
	public bool[] m_Shoots = new bool[] { false, false };
    // Internal reference to the Thread and the object that runs in it.
    protected Thread thread;
    protected SerialThreadLines serialThread;
	// Regex for comparison
	private Regex regexColorCode = new Regex("^#[a-fA-F0-9]{6}$");
...
//        Debug.Log(message);
        ...
        else
        {
			...
			if (regexColorCode.IsMatch(message.Trim()))
			{   
				gameManager.RecolorTanks (team, message.Trim ());
			}
        }
