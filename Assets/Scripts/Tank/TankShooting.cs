...
public class TankShooting : MonoBehaviour
{
	...
	// Prefab of the shell/bullet.
	public Rigidbody m_AltProjectile;
	// Prefab of the alternate bullet (ultimate)
	public GameObject m_RocketProp;
	// Prefab of the rocket prop
	public Transform m_CannonTransform;
	// A child of the tank where the rocketprops are spawned.
	...
	public GameObject m_UltimateEffect;
	// Used to reference the bubble/particle script


	private string m_FireButton;
	// The input axis that is used for launching shells.
	private string m_MovementAxisName;
	// The input axis used for moving the tank
	private string m_AimAxisName;
	// The input axis used for aiming the cannon
	...
	// Whether the shell is currently charging.
	private SerialController m_SerialController;
	// Reference to the serialcontroller
	private string m_Controller;
	// Reference to the control settings in GameManager
	private int m_RocketNumber;
	// Stores how many rockets have been fired
	private float m_Cooldown;
	// Stores the cooldown on firing.
	private bool m_CanShoot;
	// Stores whether the tank can fire.
	private float m_RocketCooldown;
	// Stores when a new rocket will be spawned
	private int m_PlayerTeamID;
	// Stores whether the player is 0 or 1 in the team
	private float m_UltimateCooldown;
	// Stores ultimate cooldown
	private float m_UltimateCooldownTime = 40f;
	// Stores ultimate cooldown time
	...
	private void OnEnable ()
	{
		...
		if (tag == "B") {
			// Set special bullets left to zero
			m_BulletsLeft = 0;
		}
	}
	...
	private void Start ()
	{
		...
		// The fire axis, movement axis, turn axis and rotation axis is based on the player number.
		...
		if (tag == "C") {
			...
			m_UltimateEffect.GetComponent<Bubble> ().m_PlayerNumber = m_PlayerNumber;
		}
	...
	private void Fire ()
	{
		...
		if (tag == "A") {
			...
			Rigidbody temp = m_Projectile;
			if (m_BulletsLeft > 1) {
				temp = m_AltProjectile;
				m_BulletsLeft -= 1;
			} else {
				if (m_UltimateEffect.activeInHierarchy) {
					m_UltimateEffect.SetActive (false);
				}
			} 

			// Create an instance of the bullet and store a reference to it's rigidbody.
			Rigidbody bulletInstance =
				Instantiate (temp, m_FireTransform.position, m_FireTransform.rotation) as Rigidbody;
	...
	{
		m_UltimateEffect.SetActive (true);
		if (tag == "A") {
			m_UltimateEffect.GetComponent<ParticleSystem> ().Play ();
			GameObject.Find ("GameManager").GetComponent<GameManager> ().AUlti (m_PlayerNumber);
		}
		if (tag == "B") {
			m_BulletsLeft += 16;
		}	
